...
const QuizifyChat = {
    ...
    async sendChatMessage() {
        ...
        if (!user) {
            this._app.showNotification('Please log in to send messages.', 'error');
            return;
        }

        const replyingToRaw = messageInput.dataset.replyingTo;

        // Optimistically clear UI — but save text in case we need to restore
        ...
        try {
            ...
            if (replyingToRaw) {
                try { messagePayload.replyTo = JSON.parse(replyingToRaw); } catch (e) { /* ignore parse error */ }
            }

            await this._globalChatCollection.add(messagePayload);
            // Success — input already cleared above
        } catch (error) {
            console.error("Error sending chat message: ", error);
            this._app.showNotification('Message failed to send. Please try again.', 'error');
            // Restore the message so user doesn't lose their text
    ...
    listenForReactions(messageId) {
        // Avoid creating duplicate listeners — detach old one first
        if (this._reactionListeners[messageId]) {
            this._reactionListeners[messageId](); // Detach old Firestore listener
        }
